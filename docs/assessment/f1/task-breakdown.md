...
| E2 | Add one app-scoped clock and lifecycle refresh | New clock service, dependency setup, service tests | E1 | One timer updates current time; resume updates immediately; disposal releases timer/observer | `GetxService` with `WidgetsBindingObserver`; root widget-owned service | **Next executable task** |
