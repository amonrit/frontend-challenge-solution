...
| E5 | Add Home-card and details rendering and interaction gates | Shared card, details screen/controller, expiry gate, widget tests | E1, E2, E3 | Both surfaces show countdown/expired state; expired card is disabled and detail Add to bag cannot mutate cart | Per-surface adapters; shared state widget plus adapters | Complete — expired Home/details widget tests pass |
| E6 | Mount the root notice host | App root, notice-host widget, widget/service test | E3 | Each automatic cart removal produces one visible notice without coupling the clock to snackbar calls | Root host; route shell host | Complete — notice display and consumption widget test passes |
| E7 | Verify 100-countdown rebuild scope and disposal | Focused widget tests, clock instrumentation | E2, E4, E5 | One tick rebuilds countdown leaves only; 100 leaves create no timer-per-widget or post-disposal callback | Build counters; test-only observer instrumentation | Complete — 100-leaf rebuild/disposal widget test passes |
| E8 | Run end-to-end and performance verification | F-1 docs, bootstrap test, profile-mode observation evidence | E3–E7 | Targeted/full tests, analyzer, Android flow, and comparable DevTools profile evidence meet acceptance criteria | Android emulator functional run; physical-device DevTools profile run | Complete — final 16-test focused suite/analyzer and live active rail/Home/details observation recorded; DevTools acceptance evidence remains incomplete |
