...
| E3 | Add cart expiry boundary and notice event | `CartService`, cart tests, notice event type | E1, E2 | Expired add is rejected; a deal expiry removes its entire cart line and emits one notice event | Cart-owned worker; clock-owned coordinator | **Next executable task** |
