...
| T4 | Audit time-dependent consumers and preserve instant semantics | `lib/feature/home/`, model call sites, focused tests | T2 | Home filter and pickup consumers use the centralized policy; `isOpenNow`/`untilStart` do not regress | Model-only API; explicit consumer helper | Complete |
| T5 | Run verification and collect before/after evidence | `docs/assessment/106/`, `solutions.md` | T3, T4 | Focused tests, full suite, analyzer, diff check, and evidence all pass and are recorded | Local commands; simulator smoke check if needed | Complete |
