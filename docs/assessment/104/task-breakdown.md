...
| T4 | Expand regression coverage for overlap and failure edges. | `test/home_controller_test.dart` | T1–T3 | Tests cover stale refresh/load-more, repeated refresh, failed stale request, and final page state. | Controller unit tests with delayed repository; widget test only if indicator behavior needs it | Next |
