...
| F1 | Capture DevTools before/after on a mid-range Android device | `docs/assessment/105/profile-baseline.md`, `solutions.md` | USB data connection and suitable device available | Identical profile-mode scenario records frame timing, rebuilds, and memory/image-cache metrics before and after | Physical device; configured Android emulator | Follow-up — emulator VM timelines and physical Perfetto fallback traces exist; repeatable physical-device DevTools and memory evidence remain |
| F2 | Add widget tests for rebuild scope and lazy construction | `test/home_feed_list_test.dart`, `test/home_screen_rebuild_scope_test.dart` | Test seam design | Lazy feed test shows a 100-deal feed builds fewer than 100 cards in the initial viewport; rebuild-scope test proves scroll state does not rebuild `HomeFeedList` | Rebuild counters; controlled builder instrumentation | Complete |
