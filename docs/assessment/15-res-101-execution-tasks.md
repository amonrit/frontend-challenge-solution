...
| --- | --- | --- | --- | --- | --- |
| E1 | Add request-generation ownership to Search and gate result/loading mutations. | `lib/feature/search/search_deals_controller.dart` | RED stale-response test | A stale completion cannot replace the latest result or loading state. | Next |
| E2 | Run and record focused GREEN result for the stale-response test. | test and evidence docs | E1 | `flutter test test/search_deals_controller_test.dart` passes. | Blocked by E1 |
| E3 | Add clear-input regression coverage. | `test/search_deals_controller_test.dart` | E2 | A response that started before clear cannot repopulate results or restore searched state. | Blocked by E2 |
| E4 | Run automated quality gates. | tests, `solutions.md` | E3 | Focused suite, full suite, analyzer, and diff check pass. | Blocked by E3 |
| E5 | Repeat the rapid-input manual comparison. | `solutions.md`, assessment evidence | E4 | Late old responses do not replace the final query's UI state. | Blocked by E4 |
