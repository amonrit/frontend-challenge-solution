...
| E1 | Add deterministic visibility qualification | `AnalyticsService`, `analytics_service_impression_test.dart` | Initial RED | An observation at `>=0.5` produces no event before one second and exactly one required event at the controlled deadline | Service-owned deadline timer; test-only explicit due check | **Next executable task** |
| E2 | Add cancellation and session deduplication | `AnalyticsService`, focused service tests | E1 | Below-threshold, disposal, and repeated/cross-source callbacks cannot emit; first qualifying source/position for a deal is retained once | Remove observation API; generation token; map replacement | Pending |
