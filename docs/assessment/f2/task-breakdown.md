...
| E6 | Make delivery state observable if needed | `AnalyticsService`, Analytics debug screen, widget/service tests | E3, E4 | Debug output can distinguish locally recorded events from pending/in-flight/failed delivery without changing required event payload | Keep existing list only; add summary state; add delivery log | **Next executable task** |
